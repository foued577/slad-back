...
export async function refresh(req: Request, res: Response): Promise<void> {
  const token = (req.body?.refreshToken as string | undefined) ?? (req.headers["x-refresh-token"] as string | undefined);
  if (!token) {
  ...
  try {
    ...
    const { sub, role } = JSON.parse(Buffer.from(token.split(".")[1], "base64").toString()) as { sub: string; role: string };
